"""
Interchange Animation Import 테스트 스크립트 생성기

이 스크립트는 로컬(언리얼 밖)에서 실행하여 언리얼 에디터용 스크립트를 생성합니다.
TemplateProcessor를 사용하여 템플릿 기반으로 스크립트를 생성합니다.

실행 방법:
1. 터미널에서 실행: uv run python tests/ue5/test_animation_import.py
2. 생성된 파일(test_inUnreal_animation_import.py)을 언리얼 에디터에서 실행
"""
...
OUTPUT_FILE = OUTPUT_DIR / "test_inUnreal_animation_import.py"
...
def generate_unreal_script():
    ...
    # 애니메이션 임포트 템플릿 처리
    result = processor.process_interchange_animation_import_template(
