...
"""
interchangeSkeletalMeshImporter 모듈 테스트 스크립트

이 스크립트는 UE5 에디터에서 실행하여 interchangeSkeletalMeshImporter 모듈의 기능을 검증합니다.
실행 후 tests/logs/test_ue5_interchangeSkeletalMeshImporter.log 파일을 확인하세요.
...
# 로그 파일 경로 설정
LOG_FILE_PATH = PROJECT_ROOT / "tests" / "logs" / "test_ue5_interchangeSkeletalMeshImporter.log"
...
import interchangeSkeletalMeshImporter
importlib.reload(interchangeSkeletalMeshImporter)
...
def test_initialization():
    ...
    try:
        ...
        if importer.asset_type == "SkeletalMesh":
        ...
        # 테스트 케이스 4: DEFAULT_SKELETAL_MESH_PREFIX 상수
        if hasattr(importer, 'DEFAULT_SKELETAL_MESH_PREFIX'):
            logger.info(f"SUCCESS: initialization - DEFAULT_SKELETAL_MESH_PREFIX 존재: {importer.DEFAULT_SKELETAL_MESH_PREFIX}")
        else:
            logger.error("FAIL: initialization - DEFAULT_SKELETAL_MESH_PREFIX 없음")
            
    except Exception as e:
        logger.error(f"FAIL: initialization - 예외 발생: {e}")


def test_import_skeletal_mesh_interface():
    """import_skeletal_mesh 메서드 인터페이스 테스트"""
    logger.info("=== TEST: import_skeletal_mesh_interface ===")
    
    try:
        importer = InterchangeSkeletalMeshImporter()
        
        # 테스트 케이스 1: 메서드 존재 확인
        if hasattr(importer, 'import_skeletal_mesh'):
            logger.info("SUCCESS: import_skeletal_mesh_interface - import_skeletal_mesh 메서드 존재")
        else:
            logger.error("FAIL: import_skeletal_mesh_interface - import_skeletal_mesh 메서드 없음")
            return
        
        # 테스트 케이스 2: 메서드 시그니처 확인 (inspect 사용)
        import inspect
        sig = inspect.signature(importer.import_skeletal_mesh)
        params = list(sig.parameters.keys())
        
        expected_params = ['inFbxPath', 'inDestinationPath', 'inSkeletonPath', 'inAssetName', 'inDescription']
        
        if params == expected_params:
            logger.info(f"SUCCESS: import_skeletal_mesh_interface - 메서드 시그니처 일치: {params}")
        else:
            logger.error(f"FAIL: import_skeletal_mesh_interface - 메서드 시그니처 불일치. 예상: {expected_params}, 실제: {params}")
            
    except Exception as e:
        logger.error(f"FAIL: import_skeletal_mesh_interface - 예외 발생: {e}")


def test_import_skeletal_meshes_interface():
    """import_skeletal_meshes 배치 메서드 인터페이스 테스트"""
    logger.info("=== TEST: import_skeletal_meshes_interface ===")
    
    try:
        importer = InterchangeSkeletalMeshImporter()
        
        # 테스트 케이스 1: 메서드 존재 확인
        if hasattr(importer, 'import_skeletal_meshes'):
            logger.info("SUCCESS: import_skeletal_meshes_interface - import_skeletal_meshes 메서드 존재")
        else:
            logger.error("FAIL: import_skeletal_meshes_interface - import_skeletal_meshes 메서드 없음")
            return
        
        # 테스트 케이스 2: 메서드 시그니처 확인
        import inspect
        sig = inspect.signature(importer.import_skeletal_meshes)
        params = list(sig.parameters.keys())
        
        expected_params = ['inFbxPaths', 'inDestinationPaths', 'inSkeletonPaths', 'inAssetNames', 'inDescription', 'inOnAssetDone', 'inOnBatchComplete']
        
        if params == expected_params:
            logger.info(f"SUCCESS: import_skeletal_meshes_interface - 메서드 시그니처 일치: {params}")
        else:
            logger.error(f"FAIL: import_skeletal_meshes_interface - 메서드 시그니처 불일치. 예상: {expected_params}, 실제: {params}")
            
    except Exception as e:
        logger.error(f"FAIL: import_skeletal_meshes_interface - 예외 발생: {e}")
...
def test_no_external_dependencies():
    ...
    try:
        # interchangeSkeletalMeshImporter 모듈 소스 코드 확인
        import interchangeSkeletalMeshImporter as module
...
def run_all_tests():
    """모든 테스트 실행"""
    logger.info("=" * 60)
    logger.info("=== interchangeSkeletalMeshImporter 모듈 테스트 시작 ===")
    logger.info("=" * 60)
    
    try:
        test_initialization()
        test_import_skeletal_mesh_interface()
        test_import_skeletal_meshes_interface()
        ...
        unreal.log(f"interchangeSkeletalMeshImporter 테스트 완료. 로그 파일: {LOG_FILE_PATH}")
