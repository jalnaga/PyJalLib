...
class Mirror:
    """객체·뼈대·지오메트리의 미러링 기능을 제공하는 클래스. 미러링 행렬 계산과 복제, 이름 생성, 계층 연결을 담당한다."""
    ...
    def mirror_matrix(self, mAxis="x", mFlip="x", tm=None, pivotTM=None):
        ...
        Args:
            mAxis (str): 미러링 축 ("x", "y", "z")
            mFlip (str): 뒤집을 축 ("x", "y", "z", 그 외 값은 뒤집지 않음)
            tm (rt.Matrix3 | None): 변환 행렬. None이면 단위 행렬을 사용한다.
            pivotTM (rt.Matrix3 | None): 피벗 변환 행렬. None이면 단위 행렬을 사용한다.

        Returns:
            rt.Matrix3: 미러링된 변환 행렬
        """
    ...
    def apply_mirror(self, inObj, axis=1, flip=2, pivotObj=None, cloneStatus=2, negative=False):
        ...
        Args:
            inObj (rt.Node): 미러링할 객체
            axis (int): 미러링 축 인덱스 (1=x, 2=y, 3=z)
            flip (int): 뒤집기 축 인덱스 (1=x, 2=y, 3=z, 4=없음)
            pivotObj (rt.Node | None): 피벗 객체. None이면 단위 행렬을 피벗으로 사용한다.
            cloneStatus (int): 복제 상태 (1=원본 변경, 2=복제본 생성, 3=스냅샷)
            negative (bool): True면 뒤집기 없이 음수 좌표계 미러링을 수행한다.

        Returns:
            rt.Node: 미러링된 객체 (cloneStatus에 따라 원본, 복제본 또는 스냅샷)
        """
    ...
    def mirror_object(self, inObjArray, mAxis=1, pivotObj=None, cloneStatus=2):
        """객체 배열을 음수 좌표계 방식으로 미러링한다.

        Args:
            inObjArray (list[rt.Node]): 미러링할 객체 배열
            mAxis (int): 미러링 축 인덱스 (1=x, 2=y, 3=z)
            pivotObj (rt.Node | None): 피벗 객체
            cloneStatus (int): 복제 상태 (1=원본 변경, 2=복제본 생성, 3=스냅샷)

        Returns:
            list[rt.Node]: 미러링된 객체 배열
        """
        returnArray = []
    ...
    def mirror_without_negative(self, inMirrorObjArray, mAxis=1, pivotObj=None, cloneStatus=2):
        """객체 배열을 양수 좌표계 방식(미러링 축과 뒤집기 축 조합)으로 미러링한다.

        Args:
            inMirrorObjArray (list[rt.Node]): 미러링할 객체 배열
            mAxis (int): 미러링 축 조합 인덱스 (1=XY, 2=XZ, 3=YX, 4=YZ, 5=ZX, 6=ZY)
            pivotObj (rt.Node | None): 피벗 객체
            cloneStatus (int): 복제 상태 (1=원본 변경, 2=복제본 생성, 3=스냅샷)
    ...
    def mirror_bone(self, inBoneArray, mAxis=1, flipZ=False, offset=0.0):
        """뼈대 배열을 새 뼈대 생성 방식으로 미러링하고 계층을 연결한다.

        Args:
            inBoneArray (list[rt.Node]): 미러링할 뼈대 배열
            mAxis (int): 미러링 축 (1=x, 2=y, 3=z)
            flipZ (bool): True면 뼈대의 Z축 방향을 뒤집는다.
            offset (float): 미러링 위치 오프셋

        Returns:
            list[rt.Node]: 미러링되어 새로 생성된 뼈대 배열
        """
        # 계층 구조에 따라 뼈대 정렬
    ...
    def mirror_geo(self, inMirrorObjArray, mAxis=1, pivotObj=None, cloneStatus=2):
        """지오메트리 객체를 미러링하고 노멀 방향을 보정한다.

        Editable Poly/Mesh 또는 모디파이어가 있는 객체는 XForm 리셋과 노멀 뒤집기를 적용한다.
