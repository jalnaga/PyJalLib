...
class SkeletalMeshImporter(BaseImporter):
    ...
    def _create_import_task(self, inFbxFile: str, inDestinationPath: str, inSkeleton: unreal.Skeleton = None):
        """스켈레탈 메시 임포트를 위한 태스크 생성 - 지정된 스켈레톤 사용"""
        ...
        if inSkeleton is not None:
            ...
            ue5_logger.error("스켈레톤이 지정되지 않음 - 새로 생성됨")
    ...
    def import_skeletal_mesh(self, inFbxFile: str, inAssetName: str = None, inSkeleton: unreal.Skeleton = None):
