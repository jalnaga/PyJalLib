...
class InterchangeSkeletonImporter(InterchangeImporterBase):
    ...
    # 스켈레탈 메쉬 에셋 접두사 (FBX 파일명 기준)
    DEFAULT_SKELETAL_MESH_PREFIX = "SK_"
    # 스켈레톤 에셋 접두사 (스켈레톤 이름에 사용)
    DEFAULT_SKELETON_PREFIX = "SKEL_"
    ...
    def import_skeleton(
        ...
        # 스켈레탈 메쉬 에셋 이름은 FBX 파일명 그대로 사용 (SK_ 접두사)
        ...
        # 스켈레톤 이름 생성: SK_ → SKEL_ 변환
        # 예: SK_Sh_Human_M_BaseSkeleton3 → SKEL_Sh_Human_M_BaseSkeleton3
        if inAssetName.startswith(self.DEFAULT_SKELETAL_MESH_PREFIX):
            skeletonName = (
                self.DEFAULT_SKELETON_PREFIX
                + inAssetName[len(self.DEFAULT_SKELETAL_MESH_PREFIX) :]
            )
        else:
            skeletonName = self.DEFAULT_SKELETON_PREFIX + inAssetName

        unreal.log(f"[InterchangeSkeletonImporter] 스켈레톤 이름: {skeletonName}")
        ...
        if pipeline is not None:
            unreal.log(
                f"[InterchangeSkeletonImporter] 파이프라인 로드됨: {pipelinePath}"
            )
            ...
            unreal.log_warning(
                f"[InterchangeSkeletonImporter] 파이프라인 로드 실패: {pipelinePath}"
            )
        ...
        # 스켈레톤 이름 변경
        skeletonRenameData = unreal.AssetRenameData(
            importedSkeleton, inDestinationPath, skeletonName
        )
    ...
    def import_skeletons(
        ...
        """
        unreal.log(
            f"[InterchangeSkeletonImporter] 스켈레톤 배치 임포트 시작: {len(inFbxPaths)}개 파일"
        )
        ...
        for index, fbxPath in enumerate(inFbxPaths):
            try:
                ...
                unreal.log_error(
                    f"[InterchangeSkeletonImporter] 스켈레톤 임포트 실패: {fbxPath}, 에러: {e}"
                )
                results.append(
        ...
        unreal.log(
            f"[InterchangeSkeletonImporter] 스켈레톤 배치 임포트 완료: 성공 {successCount}/{len(inFbxPaths)}"
        )
