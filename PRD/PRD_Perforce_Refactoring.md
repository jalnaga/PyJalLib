...
- **성능 및 안정성**: 파일 단위 루프 호출 다수(예: `run_opened`를 파일별 호출), 배치 호출 미활용, 경로 정규화/인코딩 처리 중복.
- **유지보수성 저하**: 응집도/가독성 저하, 변경 영향 예측 어려움.
- **소비자 의존**: `AnimNameSelector`, `AnimationExporter`가 `connect`, `create_change_list`, `checkout_files`, `add_files`, `submit_change_list`, `is_file_in_perforce`, `is_file_checked_out_by_others`, `check_files_checked_out`, `sync_files` 등에 의존.
...
- **기능 요구사항**
  ...
  - 경로: 절대 경로 일관화, Windows 경로 표준화(`Path(...).resolve()` + 백슬래시), 워크스페이스 루트 캐싱.
  - 제출 후 정리: 서밋 이후 "변경 없음"(unchanged) 파일은 서버 기능으로 자동 리버트.
    - `p4 revert -a -c <submitted_cl> //...` 실행.
    - `p4 revert -a -c default //...` 실행(서밋 정책에 따라 default CL로 이동한 파일 포함 정리).
...
- **비기능 요구사항**
  ...
  - 성능: 모든 가능한 API를 배치 호출로 처리.
  - 테스트: Mock 기반 단위 테스트 + 선택적 통합 훅. I/O 의존성 최소화.
  - 가시성: 주요 API 시작/종료/예외 로깅. 이벤트 훅(옵션) 제공.
...
- 공통 에러 변환 유틸/데코레이터로 `P4Exception -> PerforceError` 일원화.
...
- 연결 확인 중복 제거(데코레이터/어댑터 가드 일원화)로 분기/오버헤드 축소.
...
- **단계 2: 체인지리스트/파일 작업(배치 우선)**
  ...
  - P4Adapter: `edit_files/add_files/delete_files/revert_files` 구현 및 Service/Fascade 위임 연결.
