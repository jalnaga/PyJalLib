...
- [ ] `src/pyjallib/max/dependent.py` 파일 생성
- [ ] `Dependent` 클래스 기본 구조 작성 (docstring, `__init__` with Layer 서비스 주입)

### Task 2: get_all_dependencies 메서드 구현
- [ ] `get_all_dependencies(inObjArray, inVisited=None)` 메서드 구현
- [ ] 재귀적 dependency 탐색 (controller, skin, parent chain)
- [ ] pymxs 함수 로컬 참조를 통한 성능 최적화

### Task 3: get_dependents 메서드 구현
- [ ] `get_dependents(inObjs)` 메서드 구현
- [ ] children 수집 (원본 리스트 확장 방식)
- [ ] DependentNodes 수집

### Task 4: get_all_related_to_export 메서드 구현
- [ ] `get_all_related_to_export(inObjs)` 메서드 구현
- [ ] get_dependents, get_all_dependencies 조합
- [ ] AddOn Helper 레이어 탐색 및 포함
- [ ] 결과 선택(rt.select) 및 반환

### Task 5: header.py에 Dependent 클래스 등록
- [ ] import 문 추가
- [ ] Header 클래스에 `dependent` 속성 초기화 (Layer 서비스 주입)

### Task 6: __init__.py에 export 추가
- [ ] `Dependent` 클래스 export 추가
...
#### 검증 항목
- [ ] 빈 선택에서 오류 없이 빈 결과 반환
- [ ] Biped 오브젝트가 결과에서 제외되는지 확인
- [ ] 순환 참조 시 무한 루프 없이 처리되는지 확인
- [ ] AddOn 레이어의 Helper들이 올바르게 포함되는지 확인
