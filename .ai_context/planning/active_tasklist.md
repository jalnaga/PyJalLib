...
- [x] **Task 1.1:** 원본 값 저장 변수 및 `_store_original_values()` 메서드 추가
...
- [x] **Task 1.2:** `restore_pipeline()` 메서드 구현
  - 저장된 원본 값으로 파이프라인 속성 복원
  - 복원 후 `_originalValues` 초기화
  - 파이프라인 에셋은 저장하지 않음 (dirty 상태 방지)

- [x] **Task 1.3:** `configure_for_*` 메서드에서 자동 원본 저장 호출
...
- [x] **Task 2.1:** `_save_imported_assets()` 헬퍼 메서드 추가
  - `unreal.EditorAssetLibrary.save_loaded_assets()` 사용
  - `only_if_is_dirty=True` 옵션 적용
  - 저장 성공/실패 로깅

- [x] **Task 2.2:** `_move_to_changelist()` 헬퍼 메서드 추가
...
- [x] **Task 3.1:** `InterchangeSkeletonImporter.import_skeleton()` 수정
...
- [x] **Task 3.2:** `InterchangeSkeletonImporter.import_skeletons()` 수정
  - 배치 임포트 완료 후 파이프라인 복원
  - 저장 및 체인지리스트 할당 로직 추가

- [x] **Task 3.3:** `InterchangeSkeletalMeshImporter.import_skeletal_mesh()` 수정
  - Task 3.1과 동일한 패턴 적용

- [x] **Task 3.4:** `InterchangeSkeletalMeshImporter.import_skeletal_meshes()` 수정
  - Task 3.2와 동일한 패턴 적용

- [x] **Task 3.5:** `InterchangeAnimationImporter.import_animation()` 수정
  - Task 3.1과 동일한 패턴 적용

- [x] **Task 3.6:** `InterchangeAnimationImporter.import_animations()` 수정
  - Task 3.2와 동일한 패턴 적용

### Phase 4: 검증

- [x] **Task 4.1:** 린터 검사 실행 (`uv run ruff check .`)
  - 모든 수정된 파일에 대해 린터 오류 없음 확인
  - 결과: 수정된 5개 파일 (interchangeAnimationImporter.py, interchangeImporterBase.py, interchangePipelineSettings.py, interchangeSkeletalMeshImporter.py, interchangeSkeletonImporter.py) 모두 통과

- [x] **Task 4.2:** 기존 테스트 호환성 확인
  - `uv run pytest` 실행
  - 결과: 기존 pytest 테스트가 없음 (테스트 파일들은 언리얼 에디터용 스크립트 생성기). 실패할 테스트 없음
